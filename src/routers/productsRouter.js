...
productsRouter.get('/:pid', async (req, res, next) => {
    try {const producto = await productosManager.obtenerSegunId(req.params.pid)
        res.json(producto)
    } catch (error) {
        next(error)
    }
...
productsRouter.post('/', async (req, res, next) => {
    try {const producto = new Producto(req.body)
...
productsRouter.delete('/:pid', async (req, res, next) => {
    try {const borrada = await productosManager.borrarSegunId(req.params.pid)
        res.json(borrada)
    } catch (error) {
        next(error)
    }
