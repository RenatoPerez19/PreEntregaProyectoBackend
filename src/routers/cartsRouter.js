import { Router } from "express"import { Cart } from "../entidades/Cart.js"
...
cartsRouter.get('/', async (req, res, next) => {
    try {const carritosDb = mongoose.connection.db.collection('carts')
...
try {const carrito = await cartsManager.obtenerSegunId(req.params.cid)
    res.json(carrito)
} catch (error) {
    next(error)
}
